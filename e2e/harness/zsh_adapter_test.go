...
func TestZshAdapterBasicCommands(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping zsh adapter test in short mode")
	}

	// Check if zsh is available
	if _, err := exec.LookPath("zsh"); err != nil {
		t.Skip("zsh not available, skipping test")
	}

	adapter := NewZshAdapter()
...
`
	...
	t.Run("execute echo", func(t *testing.T) {
		result, err := adapter.Execute("echo", []string{"hello", "zsh"})
		if err != nil {
		...
		if !contains(result.Stdout, "hello zsh") {
			t.Errorf("Stdout does not contain 'hello zsh': %q", result.Stdout)
		}
	})
...
func TestZshAdapterName(t *testing.T) {
	adapter := NewZshAdapter()
	if adapter.Name() != "zsh" {
		t.Errorf("Name() = %q, want %q", adapter.Name(), "zsh")
	}
}
