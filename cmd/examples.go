...
var exampleCatalog = map[string]exampleTopic{
	...
	"clone": {
		Name:        "clone",
		Description: "Clone a repository into a category's canonical location",
		Examples: []usageExample{
			{
				Command:       "wt clone oss timvw/wt",
				Purpose:       "Acquire the main repository for a category, ready to inspect.",
				Outcome:       "Repository cloned under the category's repo_root in an owner/repo/branch layout, left on its default branch.",
				ExitCode:      "0 on success; non-zero if the category is unknown or the clone fails.",
				TextExample:   "✓ Cloned to: ~/dev/repos/oss/timvw/wt/main\nwt navigating to: ~/dev/repos/oss/timvw/wt/main",
				PathExample:   "<repo_root>/<owner>/<repo>/<branch>",
				PathBasis:     "Derived from the category's repo_root and repo_pattern (default {.category.RepoRoot}/{.repo.Owner}/{.repo.Name}/{.branch}). {.branch} is the remote's default branch (via git ls-remote, fallback \"main\").",
				Preconditions: []string{"Category defined (builtin: work, personal, oss) with a repo_root.", "gh or glab available to resolve owner/repo (not needed for a full URL)."},
				SideEffects:   []string{"Switches the active gh account to the category's gh_auth (best-effort).", "Creates directories and runs pre_clone/post_clone hooks.", "Text mode + shellenv may auto-navigate to the clone."},
				FailureModes:  []string{"Unknown category: run with a known name.", "owner/repo unresolvable: pass a full git URL.", "Destination exists and is non-empty."},
				FollowUp:      []string{"cd into the clone and inspect", "add a worktree later with wt create/checkout"},
			},
			{
				Command:     "wt clone personal git@github.com:me/dotfiles.git ~/src/dotfiles",
			...
			{
				Command:     "wt --format json clone oss timvw/wt",
				Purpose:     "Machine-readable clone for automation.",
				Outcome:     "JSON envelope with category, url, path and navigate_to.",
				ExitCode:    "0 on success; non-zero on failure.",
				JSONExample: `{"ok":true,"command":"wt clone","data":{"status":"cloned","category":"oss","url":"git@github.com:timvw/wt.git","path":"~/dev/repos/oss/timvw/wt/main","navigate_to":"~/dev/repos/oss/timvw/wt/main"}}`,
