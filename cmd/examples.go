...
var exampleCatalog = map[string]exampleTopic{
	"checkout": {
		...
		Examples: []usageExample{
			...
			},
			{
				...
				TextExample: "Use the arrow keys to navigate: ↓ ↑\n? Select a branch:\n  ▸ feature-login\n    feature-search\n    bugfix/typo",
	...
	"clone": {
		Name:        "clone",
		Description: "Clone a repository into its canonical location",
		Examples: []usageExample{
			{
				Command:       "wt clone timvw/wt",
				Purpose:       "Acquire the main repository, ready to inspect.",
				Outcome:       "Repository cloned under repo_root in a host/owner/repo/branch layout, left on its default branch.",
				ExitCode:      "0 on success; non-zero if the repo cannot be resolved or the clone fails.",
				TextExample:   "✓ Cloned to: ~/dev/repos/github.com/timvw/wt/main\nwt navigating to: ~/dev/repos/github.com/timvw/wt/main",
				PathExample:   "<repo_root>/<host>/<owner>/<repo>/<branch>",
				PathBasis:     "Derived from repo_root and repo_pattern (default {.repoRoot}/{.repo.Host}/{.repo.Owner}/{.repo.Name}/{.branch}). {.branch} is the remote's default branch (via git ls-remote, fallback \"main\").",
				Preconditions: []string{"gh or glab available to resolve owner/repo (not needed for a full URL)."},
				SideEffects:   []string{"Creates directories and runs pre_clone/post_clone hooks.", "Text mode + shellenv may auto-navigate to the clone."},
				FailureModes:  []string{"owner/repo unresolvable: pass a full git URL.", "Source has no owner/name and no explicit dest: pass a destination.", "Destination exists and is non-empty."},
				FollowUp:      []string{"cd into the clone and inspect", "add a worktree later with wt create/checkout"},
			},
			{
				Command:     "wt clone git@github.com:me/dotfiles.git ~/src/dotfiles",
				Purpose:     "Clone a full URL to an explicit destination.",
				Outcome:     "Repository cloned at the given path; placement layout is bypassed.",
				ExitCode:    "0 on success; non-zero on failure.",
				TextExample: "✓ Cloned to: ~/src/dotfiles\nwt navigating to: ~/src/dotfiles",
				PathExample: "~/src/dotfiles (explicit destination)",
				SideEffects: []string{"Destination must not already exist and be non-empty."},
			},
			{
				Command:      "WT_CATEGORY=work wt clone acme/api",
				Purpose:      "Group clones by your own axis (category, client, year) without extra config concepts.",
				Outcome:      "Placement gains a segment from the environment variable named in repo_pattern.",
				ExitCode:     "0 on success; non-zero on failure.",
				TextExample:  "✓ Cloned to: ~/dev/repos/work/acme/api/main",
				PathExample:  "<repo_root>/<WT_CATEGORY>/<owner>/<repo>/<branch>",
				PathBasis:    "Requires repo_pattern = \"{.repoRoot}/{.env.WT_CATEGORY}/{.repo.Owner}/{.repo.Name}/{.branch}\".",
				FailureModes: []string{"Referencing an unset variable is an error: give WT_CATEGORY a default in your shell rc. An empty value collapses the segment away."},
			},
			{
				Command:     "wt --format json clone timvw/wt",
				Purpose:     "Machine-readable clone for automation.",
				Outcome:     "JSON envelope with url, path and navigate_to.",
				ExitCode:    "0 on success; non-zero on failure.",
				JSONExample: `{"ok":true,"command":"wt clone","data":{"status":"cloned","url":"git@github.com:timvw/wt.git","path":"~/dev/repos/github.com/timvw/wt/main","navigate_to":"~/dev/repos/github.com/timvw/wt/main"}}`,
				SideEffects: []string{"No auto-navigation marker in JSON mode.", "git clone progress goes to stderr; stdout stays JSON-only."},
			},
		},
	},
	"create": {
	...
	"cleanup": {
		Name:        "cleanup",
		Description: "Remove worktrees for merged or stale branches",
		Examples: []usageExample{
			{
				...
				ExitCode:     "0 on success.",
				TextExample:  "Would remove 1 worktree(s):\n  - old-feature ($WORKTREE_ROOT/<repo>/old-feature) [merged]",
			...
			{
				Command:     "wt cleanup --stale --dry-run",
				Purpose:     "Preview stale worktrees: deleted remote branches or old commits.",
				Outcome:     "Lists candidates with reasons (merged, remote deleted, inactive).",
				ExitCode:    "0 on success.",
				TextExample: "Would remove 3 worktree(s):\n  - old-feature ($WORKTREE_ROOT/<repo>/old-feature) [merged]\n  - gone-branch ($WORKTREE_ROOT/<repo>/gone-branch) [remote deleted]\n  - ancient ($WORKTREE_ROOT/<repo>/ancient) [inactive (45 days)]",
				SideEffects: []string{"No deletions in dry-run mode."},
				FollowUp:    []string{"wt cleanup --stale --force"},
				Notes:       []string{"Main/master branch is never flagged as stale."},
			},
			{
				Command:     "wt cleanup --stale --stale-days 14 --dry-run",
				Purpose:     "Detect stale worktrees with a custom inactivity threshold of 14 days.",
				Outcome:     "Lists candidates using 14-day threshold instead of the default 30.",
				ExitCode:    "0 on success.",
				TextExample: "Would remove 2 worktree(s):\n  - idle-branch ($WORKTREE_ROOT/<repo>/idle-branch) [inactive (18 days)]",
				SideEffects: []string{"No deletions in dry-run mode."},
				Notes:       []string{"Default --stale-days is 30."},
			},
			{
				Command:      "wt --format json cleanup --stale --dry-run",
				Purpose:      "Machine-readable stale detection with reasons per worktree.",
				Outcome:      "JSON envelope with worktrees array including reason field.",
				ExitCode:     "0 on success.",
				JSONExample:  `{"ok":true,"command":"wt cleanup","data":{"dry_run":true,"base":"main","worktrees":[{"branch":"old-feature","path":"$WORKTREE_ROOT/<repo>/old-feature","reason":"merged"},{"branch":"gone-branch","path":"$WORKTREE_ROOT/<repo>/gone-branch","reason":"remote deleted"}]}}`,
				SideEffects:  []string{"No deletions in dry-run mode."},
				FailureModes: []string{"In JSON mode, cleanup requires --force or --dry-run."},
			},
		},
	...
	"cd": {
		...
		Examples: []usageExample{
			{
				Command:       "wt cd feature-branch",
				Purpose:       "Navigate to the worktree that already exists for a branch.",
				Outcome:       "Shell auto-navigates to that worktree; nothing is created.",
				ExitCode:      "0 on success; non-zero if no worktree exists for the branch.",
				TextExample:   "✓ Switched to worktree: $WORKTREE_ROOT/<repo>/feature-branch\nwt navigating to: $WORKTREE_ROOT/<repo>/feature-branch",
				PathExample:   "$WORKTREE_ROOT/<repo>/feature-branch (existing only)",
				PathBasis:     "Path comes from git worktree list, not from the configured pattern.",
				Preconditions: []string{"Run inside a git repository.", "A worktree already exists for the branch."},
				SideEffects:   []string{"Text mode + shellenv may auto-navigate.", "Never creates a worktree or a branch."},
				FailureModes:  []string{"No worktree for the branch: use wt checkout <branch> to create one."},
				FollowUp:      []string{"wt list", "wt status"},
			},
			{
				Command:     "wt cd",
				Purpose:     "Interactive: fuzzy-search the worktrees that already exist.",
				Outcome:     "Presents a filterable list of existing worktrees; selecting one navigates there.",
				ExitCode:    "0 on success; non-zero if selection is cancelled.",
				TextExample: "Use the arrow keys to navigate: ↓ ↑\n? Select worktree:\n  ▸ main (current)\n    feature-login\n    bugfix/typo",
				SideEffects: []string{"Text mode + shellenv may auto-navigate."},
				Notes:       []string{"Unlike wt checkout, the list contains only branches that already have a worktree.", "Alias: wt sw"},
			},
			{
				Command:     "wt --format json cd feature-branch",
				Purpose:     "Machine-readable worktree path for automation.",
				Outcome:     "JSON envelope with path and navigate_to.",
				ExitCode:    "0 on success; non-zero if no worktree exists for the branch.",
				JSONExample: `{"ok":true,"command":"wt cd","data":{"status":"exists","branch":"feature-branch","path":"$WORKTREE_ROOT/<repo>/feature-branch","navigate_to":"$WORKTREE_ROOT/<repo>/feature-branch"}}`,
				SideEffects: []string{"No auto-navigation marker in stdout.", "Requires an explicit branch argument."},
			},
		},
	},
	"copy": {
		Name:        "copy",
		Description: "Materialise the configured untracked files into a worktree",
		Examples: []usageExample{
			{
				Command:       "wt copy feature-branch",
				Purpose:       "Re-run the [files] materialisation for a worktree, e.g. after the source .env changed.",
				Outcome:       "Files declared in [files] copy/link and .worktreeinclude exist in the worktree; existing files are left alone.",
				ExitCode:      "0 on success, including when individual files fail; non-zero if the worktree or configuration cannot be resolved.",
				TextExample:   "Copied 2 files (2 reflinked), skipped 1",
				PathExample:   "$WORKTREE_ROOT/<repo>/feature-branch (destination)",
				PathBasis:     "Destination comes from git worktree list; the source is the main worktree unless --from is given.",
				Preconditions: []string{"Run inside a git repository.", "A worktree already exists for the branch.", "[files] or a .worktreeinclude declares something to copy."},
				SideEffects:   []string{"Creates files and directories in the destination worktree only.", "Never overwrites an existing destination file without --force.", "Never touches tracked files: candidates come from git ls-files --others --ignored."},
				FailureModes:  []string{"No worktree for the branch: use wt checkout <branch> to create one.", "Pattern contains .. or is absolute: rejected naming the pattern.", "WT_FILES_DISABLED=1 is set: the command errors rather than silently doing nothing.", "Unreadable source file: reported as failed on stderr; the run continues."},
				FollowUp:      []string{"wt info", "wt copy feature-branch --force"},
				Notes:         []string{"The same materialisation runs automatically on create/checkout/pr/mr.", "Reflink is used on APFS, Btrfs and XFS; a buffered copy elsewhere."},
			},
			{
				Command:     "wt copy --dry-run",
				Purpose:     "See exactly what would be materialised into the current worktree, changing nothing.",
				Outcome:     "One line per path with the verb and the reason or method; the filesystem is untouched.",
				ExitCode:    "0 on success; non-zero if the destination cannot be resolved.",
				TextExample: "would copy   .env                          (reflink)\nwould copy   .claude/settings.local.json  (reflink)\nwould skip   .envrc                       (exists)",
				SideEffects: []string{"None: no file is created, removed or modified."},
				Notes:       []string{"The predicted method matches what a real run will use, so a dry run tells you whether you get a reflink."},
			},
			{
				Command:     "wt copy feature-branch --from other-branch",
				Purpose:     "Seed a worktree from a sibling worktree instead of from the main one.",
				Outcome:     "Files are taken from the named worktree; --from also accepts a plain directory path.",
				ExitCode:    "0 on success; non-zero if --from is neither a worktree branch nor an existing directory.",
				TextExample: "Copied 1 file (1 reflinked)",
				Notes:       []string{"The source worktree only sees ignore rules that are committed, so a local-only .gitignore yields no candidates there."},
			},
			{
				Command:     "wt --format json copy feature-branch",
				Purpose:     "Machine-readable materialisation for automation.",
				Outcome:     "JSON envelope with source, destination, a summary tally and one entry per path.",
				ExitCode:    "0 on success; non-zero on failure.",
				JSONExample: `{"ok":true,"command":"wt copy","data":{"source":"/repos/wt","destination":"$WORKTREE_ROOT/<repo>/feature-branch","dry_run":false,"summary":{"copied":1,"reflinked":1,"linked":0,"skipped":0,"failed":0},"files":[{"path":".env","action":"copied","method":"reflink","bytes":42}]}}`,
				SideEffects: []string{"Requires an explicit branch argument.", "No progress output; stdout stays JSON-only."},
				FollowUp:    []string{"Read data.summary.failed to decide whether to inspect data.files."},
			},
		},
	},
	"default": {
