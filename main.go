...
Examples:
  ...
  wt init --uninstall  # Remove wt configuration from shell`,
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if initUninstall {
			if err := removeShellConfig(configPath, shell); err != nil {
		...
		if err := installShellConfig(configPath, shell); err != nil {
			fmt.Fprintf(os.Stderr, "Error: %v\n", err)
			os.Exit(1)
		}
	},
...
func detectShell(args []string) string {
	...
	if len(args) > 0 {
		...
		if shell == "bash" || shell == "zsh" || shell == "powershell" || shell == "pwsh" {
			if shell == "pwsh" {
				return "powershell"
			}
			return shell
...
func installShellConfig(configPath, shell string) error {
	...
	if strings.Contains(existingStr, markerStart) {
		...
		if endIdx > startIdx {
			...
			fmt.Printf("✓ Updated wt configuration in %s\n", configPath)
			return nil
		}
	}
	...
	fmt.Printf("✓ Added wt shell integration to %s\n", configPath)
	if !initNoPrompt {
		fmt.Println()
...
func removeShellConfig(configPath, shell string) error {
	existing, err := os.ReadFile(configPath)
	...
	after := strings.TrimLeft(existingStr[endIdx:], "\n")
	...
	fmt.Printf("✓ Removed wt configuration from %s\n", configPath)
	return nil
}
